...
Requirements:
    ...
    - Similar models of flatbed scanners; coming from the same manufacturer
...
# 2.1. CHECKING CONNECTED SCANNERS AND LINKING THEM
# =================================================
...
def scan(scanner_name, scanner_nickname, scantype, resolution, image_name, width, height, left, top, format_image,
         save_directory):
...
def warmup_photo(scanner_name, warmup_image_name, warmup_temp_folder):
    ...
    if warmup_test == False:
        suffix = ".png"
    elif warmup_test == True:
        suffix = ".tif"

    warmup_path = f"{warmup_temp_folder}/{warmup_image_name}{suffix}"

    if warmup_test == True:
        warmup_resolution = warmup_resolution_test
    else:
...
def run_selected_scanners():
    ...
    for scanner_info in scanner_vars:
        ...
        if scanner_var.get():
            ...
            scanner_infos.append((scanner_instance.libsane_name, scanner_instance.nickname, scantype, resolution,
                                  image_name, width, height, left, top, format_image, save_directory))
...
def preview_selected():
    ...
    for scanner_info in scanner_vars:
        ...
        if scanner_var.get():
            ...
            scanner_infos.append((scanner_instance.libsane_name, scanner_instance.nickname, preview_image_name, width,
                                  height, left, top, scantype, preview_folder))
...
def warmup_photo_selected():
    ...
    if warmup_test == True:
        warmup_temp_folder = os.path.join(warmup_folder, "warmup_test")
    else:
...
# 3.2. CREATE AND ARRANGE SUBFRAMES FOR EACH CONNECTED SCANNER
# ===========================================================
...
for scanner_instance in scanner_instances:
    ...
    scanner_vars.append((scanner_var, scanner_instance, scantype_var, resolution_var, image_name_var, width_var,
                         height_var, left_var, top_var))
...
# 3.3. CREATE BUTTONS TO SELECT FOLDER AND TRIGGER ACTIONS
# ========================================================
...
warmup_photo_button = ttk.Button(root, text="Warm Up (photos)", style='Accent.TButton',
                                 command=lambda: warmup_photo_selected())
...
warmup_glass_button = ttk.Button(root, text="Warm Up \n(glass plates)", style='Accent.TButton',
                                 command=lambda: warmup_glass_selected())
